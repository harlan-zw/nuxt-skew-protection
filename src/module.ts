...
declare module 'nitropack/types' {
  interface NitroRuntimeHooks {
    ...
    // event is H3Event for SSE (via POST), { headers } for WebSocket (no H3Event available)
    'skew:subscribe-stats': (payload: { id: string, event?: H3Event | { headers?: Headers } }) => void
    // event is H3Event for SSE (via POST), { headers } for WebSocket (no H3Event available)
    'skew:authorize-stats': (payload: { event?: H3Event | { headers?: Headers }, authorize: () => void }) => void
