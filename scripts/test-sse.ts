...
async function deploy(deploymentId: string, port: number, cleanStorage = false): Promise<DeploymentInfo> {
  ...
  const sseUrl = `${serverUrl}/_skew/updates`
...
class SSEClient {
  ...
  constructor(baseUrl: string, clientVersion: string) {
    this.url = `${baseUrl}?version=${encodeURIComponent(clientVersion)}`
...
async function main() {
  ...
  try {
    ...
    await runTest('Connect client with old version and verify immediate version-update', async () => {
      ...
      await waitForMessage(client2, 'connected', 3000)

      // Wait for version-update (should come immediately after connection)
      const versionUpdate = await waitForMessage(client2, 'version-update', 3000)

      if (!versionUpdate.version) {
        throw new Error('version-update message missing version field')
      }

      if (versionUpdate.version !== deployment2.deploymentId) {
        throw new Error(`Expected version ${deployment2.deploymentId}, got ${versionUpdate.version}`)
      }

      log(`  ✅ Client with old version immediately received version-update`, 'green')
