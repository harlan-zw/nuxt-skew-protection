...
The module does this automatically when [Vercel](https://vercel.com) exposes `VERCEL_SKEW_PROTECTION_ENABLED=1`. Vercel's native skew protection handles old deployment assets, while this module continues to detect updates and show notifications. Set `bundleAssets` explicitly to override the provider default.
