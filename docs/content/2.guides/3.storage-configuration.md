...
For serverless deployments like Netlify or Vercel, [Upstash](https://upstash.com/) provides a Redis-compatible database with an HTTP API.
...
In cases where you're already using a CDN with long cache times on `/_nuxt/**` assets, you may want to disable persistent storage using [`bundlePreviousDeploymentChunks`](/docs/skew-protection/api/config#bundlepreviousdeploymentchunks-boolean).
