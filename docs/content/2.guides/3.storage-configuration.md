...
[Cloudflare](https://cloudflare.com) and NuxtHub build environments do not guarantee that `node_modules/.cache` survives across deployments. Use one of these options:

- Persist `node_modules/.cache/nuxt-seo` with an explicit CI cache.
- Use a dedicated Cloudflare KV namespace through `cloudflare-kv-binding`.

Cloudflare KV is eventually consistent and bundling writes every retained client asset. Review KV quotas, value size limits, and propagation behavior for your generated output. See [Cloudflare Deployment](/docs/skew-protection/providers/cloudflare) for the exact `wrangler.jsonc` and module configuration.
