...
In cases where you're already using a CDN with long cache times on `/_nuxt/**` assets, you may want to disable persistent storage using [`bundleAssets`](/docs/skew-protection/api/config#bundleassets-boolean).

When [Vercel](https://vercel.com) exposes `VERCEL_SKEW_PROTECTION_ENABLED=1`, the module disables only asset persistence and restoration. It continues to track build metadata and deleted chunks. Set `bundleAssets` explicitly to override the provider default.

Nitro owns the `__vdpl` cookie when its native Vercel integration is active. That cookie pins same-origin polling, SSE, and [WebSocket](https://developer.mozilla.org/en-US/docs/Web/API/WebSocket) requests to the deployment that served the page, so those strategies cannot discover a newer deployment until the pin ends. Use an external update adapter when pinned clients need immediate notifications.
