...
[Pusher event data is limited to 10 KB](https://pusher.com/docs/channels/library_auth_reference/rest-api/). [Ably messages default to a 64 KiB limit](https://ably.com/docs/platform/pricing/limits). The built-in adapters use conservative payload thresholds. When a manifest exceeds its threshold, the adapter sends only its ID and timestamp. Clients still receive the update notification, but deleted chunk metadata is unavailable for that update.
