...
The default `prompt` strategy mounts the built-in notification for the whole app.
It opens the configured SSE or WebSocket connection.

If you need route-specific connections, set `reloadStrategy: false`.
Then mount `<SkewNotification>`{lang="html"} or `useSkewProtection()`{lang="ts"} only where needed.
...
First disable the built-in notification:
...
Then add `<SkewNotification>`{lang="html"} or `useSkewProtection()`{lang="ts"} where needed:
