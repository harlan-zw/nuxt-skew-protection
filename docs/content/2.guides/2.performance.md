...
The module does **not establish connections until** [`<SkewNotification>`{lang="html"}](/docs/skew-protection/api/skew-notification) or [`useSkewProtection()`{lang="ts"}](/docs/skew-protection/api/use-skew-protection) mounts. The plugin only sets up infrastructure; actual SSE/WebSocket connections happen when a component using skew protection mounts.

This means you can control exactly when the module creates connections by placing `<SkewNotification>`{lang="html"} only on specific pages.
