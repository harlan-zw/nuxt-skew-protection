...
Track which pages users are viewing. Use route data to send invalidation messages to users on specific pages.
...
**Authorization hook.** The server only sends stats to connections that pass authorization. Implement the `skew:authorize-stats` hook:
