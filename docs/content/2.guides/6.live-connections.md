...
relatedPages:
  - path: /docs/skew-protection/guides/route-tracking
    title: Tracking User Pages
  - path: /docs/skew-protection/api/use-active-connections
    title: useActiveConnections()
---

Track active connections and the versions users run as a deployment rolls out.
...
**Cloudflare with nuxt-auth-utils.** For [WebSocket](https://developer.mozilla.org/en-US/docs/Web/API/WebSocket) connections (e.g., with `cloudflare-durable` preset), the `event` object contains only `{ headers: Headers }`. Unseal the session cookie manually:
...
export default defineNitroPlugin((nitroApp) => {
  nitroApp.hooks.hook('skew:authorize-stats', async ({ event, authorize }) => {
    ...
    const session = await getUserSession(event).catch(() => {
      // WebSocket events lack the H3 context required by getUserSession.
      return null
    })
    ...
    if (event?.headers instanceof Headers) {
      ...
      if (match?.[1]) {
        ...
        if (password) {
          const unsealed = await unsealSession(null as any, { password }, decodeURIComponent(match[1])).catch(() => {
            // Invalid or expired cookies represent an unauthenticated connection.
            return null
          })
...
::note
The key difference: `getUserSession()`{lang="ts"} returns `{ user: { email } }` while `unsealSession()`{lang="ts"} returns `{ data: { user: { email } } }`.
::
...
|----------|------|-------------|
| `total` | `ComputedRef<number>`{lang="html"} | Total active connections |
| `versions` | `ComputedRef<Record<string, number>>`{lang="html"} | Map of buildId → connection count |
| `routes` | `ComputedRef<Record<string, number>>`{lang="html"} | Map of route → connection count (requires `routeTracking`) |
| `connections` | `ComputedRef<ConnectionInfo[]>`{lang="html"} | Individual connections with `id`, `version`, `route`, and `ip` (requires `ipTracking` for IP) |
| `yourId` | `ComputedRef<string \| undefined>`{lang="html"} | Your connection ID to identify yourself in the list |
| `authorized` | `ComputedRef<boolean \| null>`{lang="html"} | Whether the connection is authorized for stats (`null` while pending) |
...
Use hooks to handle connection messages and lifecycle events directly.
