...
Use `useSkewProtection()`{lang="ts"} to read version state, manage connections, and handle updates.
...
The `lazy`{lang="ts"} option (`boolean`, default `false`) controls whether the composable establishes the connection automatically when called in a mounted component. When `true`, you must call `connect()`{lang="ts"} manually.
...
If an update was already detected, registration immediately calls your callback with the retained manifest.
...
For example, a user is reading a blog post when you deploy a homepage change.
If their loaded chunks stay unchanged, the callback does not run.

A service worker tracks the loaded modules.

If chunk invalidation was already detected, registration immediately calls your callback with the retained payload.
