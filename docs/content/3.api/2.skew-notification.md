...
A headless Vue component that provides notification state and actions. You supply the template and styles.

::note
This component uses `useSkewProtection()`{lang="ts"} internally and auto-connects on mount. For manual connection control (lazy mode), use the composable directly with `useSkewProtection({ lazy: true })`{lang="ts"}.
::
...
Read these props from the scoped slot:
...
Dismiss the current update across notification components in this app.
The dismissal survives component remounts. A later deployment can show a new notification.
...
Calls `window.location.reload()`{lang="ts"}.
...
The `ChunksOutdatedPayload` from the chunk invalidation event.
...
The `forceOpen`{lang="ts"} prop (`boolean`, default `undefined`) forces the notification open for testing/debugging.
...
See [UI Examples](/docs/skew-protection/guides/ui-examples) for Nuxt UI and CSS templates.
