...
Use `useActiveConnections()`{lang="ts"} to read live SSE/WebSocket connection counts and version distribution.
...
::note
Implement `skew:authorize-stats` to choose which connections can receive stats.
::
...
**Authorization hook.** The server only sends stats to authorized connections. Implement the `skew:authorize-stats` hook in a server plugin:
...
- Type: `ComputedRef<number>`{lang="html"}
...
- Type: `ComputedRef<Record<string, number>>`{lang="html"}
...
- Type: `ComputedRef<ConnectionInfo[]>`{lang="html"}
...
- Type: `ComputedRef<string | undefined>`{lang="html"}
...
- Type: `ComputedRef<boolean | null>`{lang="html"}
