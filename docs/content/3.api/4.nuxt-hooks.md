...
## `'app:manifest:update'`{lang="ts"}

Nuxt emits this hook when it finds a newer deployment manifest. The module uses the same hook for polling, SSE, WebSocket, adapter, and sibling tab updates.

```ts [plugins/deployment-update.client.ts]
