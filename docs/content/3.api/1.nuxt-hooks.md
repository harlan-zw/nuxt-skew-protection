...
::callout{type="info"}
Stats messages are only sent to connections that pass the `skew:authorize-stats` hook. See [Live Connections](/docs/skew-protection/guides/live-connections#authorization) for setup.
::
...
Customize WebSocket connection settings. The `options` type extends [`UseWebSocketOptions`](https://vueuse.org/core/useWebSocket/) from VueUse.
