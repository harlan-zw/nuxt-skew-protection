...
- Default: `true`{lang="ts"}, or `false`{lang="ts"} when `VERCEL_SKEW_PROTECTION_ENABLED=1`
...
::note
When [Vercel](https://vercel.com) sets `VERCEL_SKEW_PROTECTION_ENABLED=1`, the module defaults `bundleAssets` to `false`. Build metadata and deleted-chunk tracking stay enabled. Nitro owns the deployment pin. Same-origin update strategies remain pinned to the current deployment, so use an external adapter for immediate cross-deployment notifications. An explicit `bundleAssets` value always takes precedence.
::
