...
::note
When [Vercel](https://vercel.com) sets `VERCEL_SKEW_PROTECTION_ENABLED=1`, the module defaults `bundleAssets` to `false`. Update detection and notifications stay enabled. An explicit `bundleAssets` value always takes precedence.
::
