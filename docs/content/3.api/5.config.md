...
### `updateStrategy: false | 'polling' | 'sse' | 'ws' | SkewAdapter`{lang="ts"}

- Default: Auto-detected based on platform

Strategy for checking for version updates.

Set `false` when provider affinity protects active sessions and no unpinned update manifest is available.

See [Update Strategies](/docs/skew-protection/guides/update-strategies) for details.

### `mode: 'portable' | 'native' | HybridMode`{lang="ts"}

- Default: native mode on [Vercel](https://vercel.com) or [Netlify](https://netlify.com) when provider affinity is available; portable mode elsewhere

Portable mode retains old build assets and reloads clients when it detects a deployment. It does not retain old server endpoints.

Native mode delegates deployment affinity to the provider. Hybrid mode keeps native affinity but reads the latest Nuxt manifest from an unpinned URL:
...
The discovery response must allow browser access and contain a valid `NuxtAppManifestMeta` with an `id`.
...
### `cookie: false | CookieOptions`{lang="ts"}
...
Set `cookie: false` when your application cannot write the version cookie. Transport connections still use the client build ID.

### `bundleAssets: boolean`{lang="ts"}

- Default: `true`{lang="ts"} in portable mode

Stores previous immutable build assets and restores retained URLs into the new deployment output. Native and hybrid modes always skip asset bundling.
...
Disable this only when a CDN or native affinity layer retains every old build asset for the full support window.

If provider affinity is active, the module keeps build metadata but skips persistent asset storage. Set `mode: 'portable'` to use module-managed retention.

### `trackBuildMetadata: boolean`{lang="ts"}

- Default: `true`{lang="ts"}

Persists release metadata and augments Nuxt build manifests independently of `bundleAssets`. Keep this enabled when a provider retains old asset bytes.
...
The module disables process-local statistics on serverless and stateless edge presets. Use a persistent Node process or the `cloudflare-durable` preset for aggregate connection counts.
