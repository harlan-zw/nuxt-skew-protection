...
Same-origin update strategies remain pinned to the current deployment. Use an external adapter for immediate cross-deployment notifications. Adapter messages carry the new build manifest when it fits the provider's payload limit. Oversized manifests fall back to an ID and timestamp notification. Setting `bundleAssets: false` explicitly disables asset and metadata persistence. Set `trackBuildMetadata: true` to retain metadata.
::
