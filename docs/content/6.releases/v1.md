...
relatedPages:
  - path: /docs/skew-protection/getting-started/introduction
...
Nuxt Skew Protection v1 is the module's first stable release. Choose when outdated clients reload and read version state on the client and server.

Several hooks and options have shorter names. Check the [breaking changes](#breaking-changes) before upgrading.

Run the codemod below to migrate your project. For a guided migration, use the [LLM upgrade prompt](#llm-upgrade-prompt) with Claude or [ChatGPT](https://chatgpt.com).
...
The `isRollback` computed ref is `true` when the server version is **older** than the client version. Use it to detect rollbacks.

## 🛠️ DevTools Panel

Inspect skew protection state in the new Nuxt DevTools panel. It has four tabs:
...
Tabs share detected deployments through `BroadcastChannel`, reducing duplicate polling. This is enabled by default. Set `multiTab: false` to disable it.

## 🔄 Reload Strategies

Previously the module always showed a notification and waited for the user to click reload. Now you choose the behavior that fits your app with a single config option.
...
The minimum version changed for this dependency. Check its release notes before upgrading:
...
**Action:** No code changes needed unless you explicitly rely on the old defaults. If you need the old behavior, set them in config:
...
- `getClientVersion(event)`{lang="ts"} server utility ([`4b6a9fe`](https://github.com/harlan-zw/nuxt-skew-protection/commit/4b6a9fe))
