...
Configure [persistent storage](/docs/skew-protection/guides/storage-configuration) so that Nuxt build assets
...
### 3. Choose the Notification UI

The default `prompt` strategy is headless. It detects updates without adding an interface.

Mount the ready-made notification in `app.vue` or a layout:
...
Use `<SkewNotification>`{lang="html"} when you want a custom interface.
This example uses Nuxt UI:
...
If you mounted a notification, deploy an `app.vue` change to test it.
