...
Use **Cloudflare Durable Objects** for real-time update notifications via WebSockets.
...
When deploying directly to Cloudflare, local wrangler writes build assets to your default Cloudflare KV namespace.
Filesystem storage with CI/CD caching is also available, including for NuxtHub.
...
Configure the `cloudflare-durable` preset and enable WebSockets:
...
[NuxtHub](https://hub.nuxt.com) manages Cloudflare resources for your deployment. The module uses **filesystem storage** by default with NuxtHub.
